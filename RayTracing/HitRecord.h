...
class HitRecord{
	public:
		...
		HitRecord(float t1, glm::vec4 p1, glm::vec4 normal1, graphics::Material material1, float sText1, float tText1, graphics::Texture *texture1);
		~HitRecord();
		void setT(float t1);
		void setP(glm::vec4 p1);
		void setNormal(glm::vec4 normal1);
		void setMaterial(graphics::Material material1);
		void setTextCoords(float sText1, float tText1);
